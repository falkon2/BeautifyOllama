...
def load_config():
    ...
    default_config = {
        'model': 'qwen3:0.6b',  # Use an available model
        'searxng_instances': [
            'http://localhost:32768',  # Local Docker SearxNG instance
...
"""
    ...
    def model_response(self, model: str, message: str, max_retries: int = 3, thinking: bool = False) -> Optional[str]:
        """Get response from Ollama model with error handling and thinking mode control"""
        for attempt in range(max_retries):
            try:
                # Create conversation with thinking mode setting first
                messages = []
                
                # First message: Set the thinking mode
                if thinking:
                    messages.append({'role': 'user', 'content': '/set think'})
                    messages.append({'role': 'assistant', 'content': "Set 'think' mode."})
                else:
                    messages.append({'role': 'user', 'content': '/set nothink'})
                    messages.append({'role': 'assistant', 'content': "Set 'nothink' mode."})
                
                # Second message: Send the actual user message
                messages.append({'role': 'user', 'content': message})
                
                # Get the response to the actual message
    ...
    def generate_search_query(self, question: str, thinking: bool = False) -> Optional[str]:
...
User's question: """ + question
        ...
        return self.model_response(CONFIG['model'], prompt, thinking=thinking)

    def select_best_result(self, question: str, query: str, results: List[Dict], thinking: bool = False) -> Optional[Tuple[str, str]]:
...
URL: [exact URL from results]"""
        ...
        response = self.model_response(CONFIG['model'], prompt, thinking=thinking)
    ...
    def generate_final_answer(self, question: str, query: str, title: str, content: str, thinking: bool = False) -> None:
...
- Be helpful and informative"""
        ...
        # Set thinking mode and get response
        try:
            # Create conversation with thinking mode setting first
            messages = []
            ...
            # Second message: Send the actual prompt
            messages.append({'role': 'user', 'content': prompt})
            
            # Stream the response for the actual prompt
            print(f"{Colors.GREEN}🤖 Assistant:{Colors.END}", end=" ")
            for chunk in ollama.chat(model=CONFIG['model'], messages=messages, stream=True):
                content_chunk = chunk['message']['content']
                print(content_chunk, end='', flush=True)
                time.sleep(CONFIG.get('streaming_delay', 0.02))
            print()  # New line after streaming
            
        except Exception as e:
            print(f"\n{Colors.RED}❌ Error during response generation: {str(e)}{Colors.END}")
...
def search_and_return_json(assistant: WebSearchAssistant, user_query: str, thinking: bool = False) -> Dict:
    """Perform search and return structured JSON results for API integration"""
    result = {
        'success': False,
    ...
    try:
        ...
        search_query = assistant.generate_search_query(user_query)
        if not search_query:
            search_query = user_query  # Fallback
        
        result['search_queries'] = [search_query]
        
        # Step 2: Search the web
        search_results = assistant.browse_web(search_query)
        ...
        best_result = assistant.select_best_result(user_query, search_query, search_results)
        if not best_result:
            result.update({
                'success': True,
                'summaries': [{
                    'query': search_query,
                    'summary': 'Could not select a relevant result.',
                    'sources': []
                }],
                'sources': []
            })
        ...
        if not content:
            result.update({
                ...
                'summaries': [{
                    'query': search_query,
                    'summary': 'Could not retrieve page content.',
                    'sources': [url],
                    'title': title
                }],
                'sources': [url]
        ...
        # Step 5: Generate summary (using existing method but capture output)
        # Create a simple summary instead of the full streaming response
        prompt = f"""You are a knowledgeable assistant providing accurate information based on web content.
...
- Be helpful and informative"""
        ...
        if thinking:
            summary = assistant.model_response(CONFIG['model'], prompt, thinking=True)
        else:
            summary = assistant.model_response(CONFIG['model'], prompt, thinking=False)
        
        if not summary:
            summary = f"Retrieved content from {title}: {content[:500]}..."
...
def main():
    ...
    parser.add_argument('--json', action='store_true', help='Output results in JSON format (for API integration)')
    ...
    # JSON mode for API integration (no banner, colors, or interactive prompts)
    if args.json:
        ...
        if not args.query:
            result = {
                'success': False,
                'error': 'Query is required in JSON mode',
                'user_query': ''
            }
            print(json.dumps(result))
            sys.exit(1)
        ...
        result = search_and_return_json(assistant, args.query, args.thinking)
        print(json.dumps(result, indent=2 if args.thinking else None))
        return
